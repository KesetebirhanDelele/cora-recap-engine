...
**Artifact**: `dashboard-ui/` directory
**Dependencies**: All API phases complete and stable

### Structure (current, post Phase 7)
```
dashboard-ui/
  app/
    page.tsx                    ← home (health tiles + nav groups)
    health/page.tsx
    activity/page.tsx
    exceptions/page.tsx         ← exception queue, grouped by type
    queue/page.tsx
    alerts/page.tsx
    crm-health/page.tsx
    lead/[id]/page.tsx          ← pipeline trace
    voice-performance/page.tsx  ← "use client", date filter, no scroll
    ai-performance/page.tsx     ← "use client", date filter, AI-only metrics
    conversion-funnel/page.tsx  ← "use client", date filter, funnel + trends
  components/
    HealthTiles.tsx
    ActivityFeed.tsx
    PipelineTrace.tsx
    ExceptionQueue.tsx          ← grouped view, bulk-ignore per type
    CampaignFunnel.tsx          ← intent + consent charts (used by ai-performance)
    QueueTable.tsx
    AlertBanner.tsx
    AlertsClient.tsx
    NavigationCard.tsx
    PageShell.tsx
    voice/
      DateRangePicker.tsx       ← shared date filter component
      KpiSidebar.tsx
      TrendsChart.tsx
      WowWaterfall.tsx
...
### Navigation groups (home page)
- **Operations**: Live Activity, Exceptions, Queue Health, Alerts
- **Analytics**: Voice Performance, AI Performance, Conversion Funnel
- **System**: CRM Health

### Analytics page ownership (no cross-contamination)
- Voice Performance → business outcomes only
- AI Performance → AI behavior only (intent, consent, blank rate, unknown intent %)
- Conversion Funnel → drop-off analysis (Total Calls → Picked Up → Engaged → Booked)

---

## Phase 7 — Analytics v2 (completed 2026-04-06)
**Artifact**: New pages + new API endpoint
**Summary of changes**:
- Removed `/kpis` page and nav card (KPI data subsumed into Conversion Funnel and Voice Performance).
- Added `GET /dashboard/ai-timeseries` — weekly intent distribution, blank transcript %, unknown intent % per week.
- Added `GET /dashboard/exceptions` `groups` field — per-type counts without extra round-trip.
- Added `POST /dashboard/actions/bulk-ignore` — clears all open exceptions of a given type atomically.
- Created `/conversion-funnel` page: funnel bars, step conversion table, drop-off highlight, weekly trend chart (pickup + booking rates).
- Refactored `/ai-performance` page: AI quality tiles (blank rate, unknown intent %), intent + consent charts, intent → outcome table, weekly AI error trend chart.
- Added `DateRangePicker` to AI Performance and Conversion Funnel (default: last 7 days).
- Light theme applied across all pages; consistent card/border/text tokens throughout.
