...
- Shadow mode: outbound calls intercepted and logged to `shadow_actions`. SMS/email content fully generated by AI and stored to `outbound_messages` (status='shadow') — visible in Lead Journey timeline with a shadow badge. GHL field writes intercepted and logged to `shadow_actions` (action_type='ghl_contact_update') with human-readable field map visible in Lead Journey. Active-window enforcement applies identically to live and shadow mode.
