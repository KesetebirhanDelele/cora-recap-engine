"""
GhlConversationLogEvent — record of a GHL Conversations call-log write attempt.
...
TaskEvent):
  CREATE UNIQUE INDEX uq_ghl_conversation_log_events_one_success
      ON ghl_conversation_log_events(call_event_id) WHERE status = 'created';
...
class GhlConversationLogEvent(Base):
    __tablename__ = "ghl_conversation_log_events"
    __table_args__ = (
        Index("idx_ghl_conversation_log_events_call_event", "call_event_id"),
        Index("idx_ghl_conversation_log_events_status", "call_event_id", "status"),
    )
    ...
    # status: 'created' (GHL conversation message exists) | 'failed' (attempt did not produce one)
    ...
    def __repr__(self) -> str:
        return (
            f"<GhlConversationLogEvent call_event_id={self.call_event_id!r} "
