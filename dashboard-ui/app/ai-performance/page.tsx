 ...
 *
 * Sections:
 *   1. Voice Agent tabs (All / NewLead / ColdLead / Inbound)
 *   2. AI Quality tiles
...
// Voice agent tab → API campaign_name mapping
const VOICE_AGENTS: { label: string; campaign: string | null }[] = [
  { label: "All",      campaign: null },
...
export default function AiPerformancePage() {
  ...
  const [activeTab, setActiveTab] = useState(0); // index into VOICE_AGENTS
  ...
  const activeCampaign = VOICE_AGENTS[activeTab]?.campaign ?? null;

  const load = useCallback(async (from: string, to: string, campaign: string | null) => {
    setLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => { load(fromDate, toDate, activeCampaign); }, [fromDate, toDate, activeCampaign, load]);

  // Handle intent bar click — fetch drill-down calls
  async function handleIntentClick(intentRaw: string) {
    // intentRaw has spaces (from chart formatting); restore underscores
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "system-ui, -apple-system, sans-serif", color: "#1e293b" }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "#1e293b" }}>AI Performance</span>
